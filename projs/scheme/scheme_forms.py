...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        symbol, expr = expressions.first, expressions.rest.first
...
def do_and_form(expressions, env):
    ...
    if expressions.rest is nil:
        val = scheme_eval(expressions.first, env, True)         # EC
        if is_scheme_true(val): return val
        return False
    val = scheme_eval(expressions.first, env)
    if is_scheme_true(val): return do_and_form(expressions.rest, env)
    return False
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if expressions.rest is nil:
        val = scheme_eval(expressions.first, env, True)         # EC
        if is_scheme_false(val): return False
        return val
    val = scheme_eval(expressions.first, env)
    if is_scheme_false(val): return do_or_form(expressions.rest, env)
    return val
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    # BEGIN PROBLEM 14
    validate_formals(bindings.map(lambda p: p.first))
    bindings_iter = bindings
    while bindings_iter is not nil:
        kv = bindings_iter.first
        validate_form(kv, 2, 2)
        names = Pair(kv.first, names)
        vals = Pair(scheme_eval(kv.rest.first, env), vals)
        bindings_iter = bindings_iter.rest
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    if not scheme_listp(signature):
        raise SchemeError('Signature must be list')
