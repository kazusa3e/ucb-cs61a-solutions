...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            assert place.ant.can_contain(self) or self.can_contain(place.ant), \
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        iter_place, distance = self.place, 0
        assert iter_place != None
        while not iter_place.is_hive:
            if self.min_range <= distance <= self.max_range:
                bee = random_bee(iter_place.bees)
                if bee:
                    return bee
            iter_place, distance = iter_place.entrance, distance + 1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        assert self.place is not None
        if self.place.bees != [] and self.chew_timer == 0:
            bee_to_eat = random_bee(self.place.bees)
            assert bee_to_eat is not None
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if (not hasattr(gamestate, 'exist_queen_ant')) or (not gamestate.exist_queen_ant):
            gamestate.exist_queen_ant = True
    ...
    def action(self, gamestate):
        ...
        super().action(gamestate)
        assert self.place is not None
        iter_place = self.place.exit
        while iter_place is not None:
            double_ant_damage(iter_place.ant)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slow_timer != 0 and gamestate.time % 2 == 1:
            destination = self.place
        elif ((self.scary_timer != 0)
            and (not self.place.entrance.is_hive)
            and (not (self.slow_timer != 0 and gamestate.time % 2 == 1))
            ):
            self.scary_timer -= 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        def inspects_in_place(place):
            result = []
            if place.ant:
                result.append(place.ant)
                if place.ant.is_container and place.ant.ant_contained:
                    result.append(place.ant.ant_contained)
            result.extend(place.bees)
            return result

        assert self.place is not None
        result = {}

        if self.place.ant.is_container and self.place.ant.ant_contained is self:
            result[self.place.ant] = 0
        if self.place.bees != []:
            for bee in self.place.bees:
                result[bee] = 0
        iter_place, distance = self.place.entrance, 1
        while not iter_place.is_hive:
            for insect in inspects_in_place(iter_place):
                result[insect] = distance
            iter_place, distance = iter_place.entrance, distance + 1
        return result
        # END Problem Optional 2
