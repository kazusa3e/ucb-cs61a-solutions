...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal:
        if who == 0:
            points = take_turn(strategy0(score0, score1), score0, score1, dice, goal)
            score0 += points
            score0 += hog_pile(score0, score1)
        else:
            points = take_turn(strategy1(score1, score0), score1, score0, dice, goal)
            score1 += points
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    def fn(*args):
        samples = [original_function(*args) for _ in range(total_samples)]
        return sum(samples) / total_samples
    return fn
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    rolls = [averaged_dice(_, dice) for _ in range(1,11)]
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    hefty_scores = hefty_hogs(score, opponent_score)
    return 0 if hefty_scores >= threshold else num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hefty_rolls = hefty_hogs_strategy(score, opponent_score, threshold, num_rolls) # zero or n
    hefty_scores = hefty_hogs(score, opponent_score)
    if hog_pile(score + hefty_scores, opponent_score) > 0:
        return 0
    return hefty_rolls
    # END PROBLEM 11
