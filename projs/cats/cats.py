...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def helper(paras):
        words = split(remove_punctuation(lower(paras)))
        return any([(keyword in words) for keyword in topic])
    return helper
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if typed_words == [] or reference_words == []:
        return 0.0
    spellright = len([x for x in list(
        zip(typed_words, reference_words),
    ) if x[0] == x[1]])
    return spellright / len(typed_words) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    result = min(diffs, key=lambda d: d['diff'])
    return typed_word if result['diff'] > limit else result['word']
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    add = minimum_mewtations(start[1:], goal, limit - 1) + 1
    remove = minimum_mewtations(start, goal[1:], limit - 1) + 1
    subs = minimum_mewtations(start[1:], goal[1:], limit - 1) + 1
    return min(add, remove, subs)
    # END PROBLEM 7
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for ix in range(len(prompt)):
        if len(sofar) > ix and sofar[ix] == prompt[ix]:
            correct_count += 1
        else:
            break
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[
        (p[ix] - p[ix - 1]) for ix in range(1, len(p))
...
def fastest_words(match):
    ...
    for w in word_indices:
        times = [time(match, p, w) for p in player_indices]
        result[times.index(min(times))].append(word_at(match, w))
    return result
    # END PROBLEM 10
